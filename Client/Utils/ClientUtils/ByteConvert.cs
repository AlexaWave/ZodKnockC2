...
namespace Client.Utils.ClientUtils
{
    class ByteConvert : Models.Util
    {
        ...
        public override string UtilExecute(string[] opts)
        {
            try
            {
                if (opts is null) { throw new ZodException($"[*] Usage: ByteConvert [isRemote] [filePath] <timeout> <retryCount>\n"); }
                if (opts.Length < 3) { throw new ZodException($"[*] Usage: ByteConvert [isRemote] [filePath] <timeout> <retryCount>\n"); }
                ...
                } else { throw new ZodException($"[*] Usage: ByteConvert [isRemote] [filePath] <timeout> <retryCount>\n"); }

                try {
                    if (opts.Length > 3) { Timeout = Int32.Parse(opts[3]); }
                    if (opts.Length > 3 && opts.Length <= 5) { retryCount = Int32.Parse(opts[4]); }
                } catch (FormatException) { throw new ZodException($"[*] Usage: ByteConvert [isRemote] [filePath] <timeout> <retryCount>\n"); }
