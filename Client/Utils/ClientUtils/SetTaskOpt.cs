...
namespace Client.Utils
{
    class SetTaskOpt : Models.Util
    {
        ...
        public override string UtilExecute(string[] opts)
        {
            try
            {
                if (TaskName is null) { throw new ZodException($"[-] Select a task before attempting to set task options\n"); }

                if (opts is null || opts.Length != 3) { throw new ZodException($"[*] Usage: SetTaskOpt [optionName] [optionValue]\n"); }
                ...
                throw new ZodException($"[-] Option {OptName} does not exist in the current context\n");
