    ...
    class StartListener : Models.Util
    {
        ...
        public override string UtilExecute(string[] opts)
        {
            try
            {
                ...
                if (opts is null) { throw new ZodException($"[*] Usage: StartListener [ListenerName] [ListenerPort]\n"); }
                if(opts.Length > 3 || opts.Length < 3) { throw new ZodException($"[*] Usage: StartListener [ListenerName] [ListenerPort]\n"); }

                ListenerName = opts[1];
                ListenerPort = Int32.Parse(opts[2]);

                if(ListenerPort < 0 && ListenerPort > 65535) { throw new ZodException($"[-] ListenerPort must be a valid port"); }
