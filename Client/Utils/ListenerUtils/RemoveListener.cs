    ...
    class RemoveListener : Models.Util
    {
        ...
        public override string UtilExecute(string[] opts)
        {
            try{
                ...
                if (opts is null) { throw new ZodException($"[*] Usage: ViewListener [ListenerName]\n"); }

                ListenerName = opts[1];

                var _listener = ListenerList.FirstOrDefault(listener => listener.Equals(ListenerName));
                if (_listener is null) { throw new ZodException($"[-] Listener {ListenerName} does not exist\n"); }
