...
namespace Client.Utils
{
    public class RemoveImplant : Models.Util
    {
        ...
        {
            if (opts is null) { throw new ZodException($"[-] No parameters passed\nUsage: RemoveImplant [implantName]\n"); }
            if (opts.Length > 2) { throw new ZodException($"[*] Incorrect parameters passed\nUsage: RemoveImplant [implantName]\n"); }

            StringBuilder outData = new StringBuilder();

            var implantName = opts[1];
            var _implant = ImplantList.FirstOrDefault(implant => implant.Equals(implantName));
            if (_implant is null) { throw new ZodException($"[-] Implant {implantName} does not exist"); }
