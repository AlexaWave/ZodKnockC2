...
namespace Client.Utils
{
    static class UI
    {
        ...
        public static void Action(string input) {
            try
            {
                ...
                if (input is "") { throw new ZodException(""); }
                ...
                if (util is null && admutil is null) { throw new ZodException($"[-] Util {input} is invalid"); }
            ...
            catch (ZodException e) { WriteLine(e.Message); }
    ...
    {
        public static void VerifyFileExistence(string file){ if (!(File.Exists(file))) { throw new ZodException($"[-] File does not exist"); } }
        ...
        public static void VerifyURL(string file) {
            ...
            if(!URLStatus) { throw new ZodException($"[-] Invalid URL detected "); }
        }
        ...
        public static List<object> ReturnMethod(){
                ...
                if (task is null) { throw new ZodException($"[-] A task must be selected to view task options\n"); }
            ...
            while (current_retry_count >= 0 && assemBytes == null)
            {
                ...
                catch (WebException)
                {
                    if (current_retry_count == 0) { 
                        throw new ZodException($"[-] Failed to fetch {fileAddr} after {current_retry_count} attempts. Exiting...");
                    }
